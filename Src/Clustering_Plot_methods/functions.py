...
def one_hot_encoding(json_file_path: str, save_path: str, save_name: str):
    """
    One hot encoding
    """
    # Src/Clustering/middle_files/vendor_label_hash_json/vendor_label_hash_dict.json
    # Src/Clustering/middle_files/vendor_label_hash_json/vendor_label_hash_labelOver100.json
    with open(f"{json_file_path}", "r", encoding='utf-8') as f:
        vendor_label_hash = json.load(f)
    labels = []
    hashes_set = []

    for l, hashes_values in vendor_label_hash.items():
        labels.append(l)
        hashes_set.append(hashes_values)
    
    mlb = MultiLabelBinarizer(sparse_output=True)
    features = mlb.fit_transform(hashes_set)
    print(features.shape)
    with open(f"{save_path}/{save_name}_labels.txt", "w", encoding='utf-8') as f:
        for l in labels:
            f.write("%s\n" % l)

    # Src/Clustering/middle_files/one_hot_encoding_formatData/one_hot_encodingCSC.npz
    # Src/Clustering/middle_files/one_hot_encoding_formatData/vendor_label_hash_labelOver100CSC.npz
    scipy.sparse.save_npz(f"{save_path}/{save_name}_CSR.npz", features)
...
def i_min_ab_a(csr_matrix: sps.csr_matrix, logistic_alpha, s_shaped_alpha, beta, s_shaped_func=False):
    """
    ...
    if s_shaped_func:
        ...
        return intersection, logisti_similarity
...
def plot_dendrogram(save_path: str,
                    ...
                    plot_pairwise_intersection: bool =False,
                    pairwise_keyword_str: str ="all"):
    """_summary_

    Args:
        save_path (str): _description_
        label_file_path (str): _description_
        npz_filepath (str): _description_
        linkage_matrix_filepath (str): _description_
        save_name (str): _description_
        family_name_count_json_filepath (str, optional): _description_. Defaults to "".
        plot_pairwise_intersection (bool, optional): _description_. Defaults to False.
        pairwise_keyword_str (str, optional): _description_. Defaults to "all".
    """
    save_path = save_path
    label_file_path = label_file_path
    npz_filepath = npz_filepath
    linkage_matrix_filepath = linkage_matrix_filepath
    save_name = save_name
    family_name_count_json_filepath = family_name_count_json_filepath
    plot_pairwise_intersection = plot_pairwise_intersection
    pairwise_keyword_str = pairwise_keyword_str
    ...
    if isinstance(family_name_count_json_filepath, str):
        if len(family_name_count_json_filepath) > 0:
            ...
            family_names = []
    elif isinstance(family_name_count_json_filepath, list):
        family_names = family_name_count_json_filepath
        # family_names = ["gafgyt", "coinminer", "oinminer", 'sabsik', 'hajime', 'tsunami', 'mirai']
    
    if len(family_names) > 6:
        # colors = random.sample(list(matplotlib.colors.cnames.keys()), len(family_names))
        radii = np.random.random(len(family_names))
        colors = [
            "#%02x%02x%02x" % (int(r), int(g), int(b)) for r, g, b, _ in 255*matplotlib.cm.plasma(matplotlib.colors.Normalize()(radii))
        ]
        colormap = {}
    ...
    if plot_pairwise_intersection:
        ...
        plot_pairwise_intersection_based_matrix(
            ...
            colormap=colormap,
            save_path = save_path,
            keywords_str=pairwise_keyword_str,
            alpha=12,
            method="Set_Dist"
        )
    print("END")
